...
class LogSetup:
    ...
    def _setup_file_handler(self, file_path) -> None:
        try:
            ...
            self._logger.warning(
                "Failed to create the debug log file: %s. Falling back to terminal output.",
                e,
            )
        else:
            handler.setLevel(logging.DEBUG)
            handler.setFormatter(LevelSwitchFormatter(DEBUG_LOG_FMT, WARN_LOG_FMT))
    ...
    def _setup_stream_handler(self) -> None:
        ...
        h_debug.setFormatter(logging.Formatter(fmt=DEBUG_LOG_FMT, datefmt="%H:%M:%S"))

        h_warn = logging.StreamHandler()
        h_warn.setLevel(logging.WARNING)
        h_warn.setFormatter(logging.Formatter(fmt=WARN_LOG_FMT, datefmt="%H:%M:%S"))
